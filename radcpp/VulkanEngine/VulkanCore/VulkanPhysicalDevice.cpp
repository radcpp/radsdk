...
VkFormat VulkanPhysicalDevice::FindFormat(ArrayRef<VkFormat> candidates, VkImageTiling tiling, VkFormatFeatureFlags features)
{
    for (VkFormat cadidate : candidates)
    {
        ...
        if ((tiling == VK_IMAGE_TILING_LINEAR) && HasBits(formatProps.linearTilingFeatures, features))
        {
            return cadidate;
        }
        else if ((tiling == VK_IMAGE_TILING_OPTIMAL) && HasBits(formatProps.optimalTilingFeatures, features))
        {
            return cadidate;
        }
    }
...
{
    return MakeRefCounted<VulkanDevice>(m_instance, this, extensionNames);
}
